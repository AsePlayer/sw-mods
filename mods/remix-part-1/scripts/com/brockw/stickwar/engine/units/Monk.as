package com.brockw.stickwar.engine.units
{
     ...
     public class Monk extends com.brockw.stickwar.engine.units.Unit
     {
          ...
          {
               return Boolean(this.isCuring) || Boolean(this.isHealing) || Boolean(this.isShielding) || isBusyForSpell;
          }
          
          public function healSpell(param1:com.brockw.stickwar.engine.units.Unit) : Boolean
          {
               if(!this.isBusy() && Boolean(this.healSpellCooldown.spellActivate(team)))
               {
                    this.isHealing = true;
                    _state = S_ATTACK;
                    hasHit = false;
          ...
          {
               if(!this.isBusy() && techTeam.tech.isResearched(Tech.MONK_CURE) && Boolean(this.cureSpellCooldown.spellActivate(team)))
               {
          ...
          {
               var _loc2_:com.brockw.stickwar.engine.units.Unit = null;
               if(!this.isSlowing && Boolean(this.slowSpellCooldown.spellActivate(this.team)))
               {
                    this.spellX = param1;
