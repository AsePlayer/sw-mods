class Giant extends Man
{
     ...
     function Giant(x, y, squad, simUnits, game)
     {
          ...
          this.isGlacialGiant = false;
          this.isKillerGiant = false;
     }
     function update()
     {
          super.update();
          this.clip.health.bar._xscale = this.health / this.MAX_HEALTH * 100;
          if(this.isGlacialGiant)
          {
               this.clip.weapon = 5;
          }
          else
          {
               this.clip.weapon = this.squad.getTechnology().getGiantClub();
          }
          if(!this.isAlive)
          {
          ...
          else if(this.isSwinging)
          {
               ...
               if(this.isHitPeriod())
               {
                    ...
                    {
                         if(this.isKillerGiant)
                         {
                              this.squad.getEnemyTeam().damageCastle(this.squad.getTechnology().getGiantAttack() * 99);
                         }
                         else
                         {
                              this.squad.getEnemyTeam().damageCastle(this.squad.getTechnology().getGiantAttack() * 4);
                         }
                         this.hasDamaged = true;
                    }
               }
     ...
     function damage(amount, direction, type)
     {
          ...
          var _loc7_ = "healthLost" + this.healthLostnum;
          this.game.screen.attachMovie("healthLost",_loc7_,10000000000040 + this.healthLostnum);
     ...
     function setAsGlacialGiant()
     {
          this.isGlacialGiant = true;
          this.health *= 1.25;
          this.MAX_HEALTH *= 1.25;
     }
     function setAsKillerGiant()
     {
          this.isKillerGiant = true;
          this.health *= 99;
          this.MAX_HEALTH *= 99;
          this.MAX_VELOCITY *= 3;
          this.MAX_ACCELERATION *= 3;
     }
     function setAsPummeler()
     {
          this.baseScale /= 3.25;
          this.GIANT_RANGE *= 3.25;
          this.isPummeler = true;
          this.health *= 15;
          this.MAX_HEALTH *= 15;
     }
}
