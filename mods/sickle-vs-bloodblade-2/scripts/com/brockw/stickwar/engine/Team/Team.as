package com.brockw.stickwar.engine.Team
{
     ...
     public class Team
     {
          ...
          private function spawnGhost(param1:StickWar) : Unit
          {
               var _loc4_:AttackMoveCommand = null;
               if(param1.map.hills.length == 0)
               {
               ...
               param1.projectileManager.initTowerSpawn(param1.map.hills[1].px,param1.map.height / 2,this,_loc5_);
               return _loc3_;
          }
               ...
               if(param1.map.hills.length != 0)
               {
                    if((_loc11_ = param1.map.hills[1].getControllingTeam(param1)) == this)
                    {
                         if(this.tech.isResearched(com.brockw.stickwar.engine.Team.Tech.TOWER_SPAWN_I))
                         {
                              if(param1.frame - this.timeSinceSpawnedUnit > this.towerSpawnDelay)
                              {
                                   this.spawnedUnit = this.spawnGhost(param1);
                              }
                         }
                    }
               }
