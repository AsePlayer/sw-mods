package com.brockw.stickwar.engine.units
{
     ...
     public class EnslavedGiant extends RangedUnit
     {
          ...
          public var enslavedgiantType:String;
          ...
          public function EnslavedGiant(param1:StickWar)
          {
               ...
               initSync();
               firstInit();
               this.enslavedgiantType = "Default";
          }
          ...
          override public function init(param1:StickWar) : void
          {
               ...
               this.loadDamage(param1.xml.xml.Order.Units.giant);
               _mc.stop();
               _mc.width *= _scale;
               _mc.height *= _scale;
               _state = S_RUN;
          ...
          override public function update(param1:StickWar) : void
          {
               ...
               if(isUC)
               {
                    _maxVelocity = param1.xml.xml.Order.Units.giant.maxVelocity * 1.2;
                    _damageToNotArmour = (Number(param1.xml.xml.Order.Units.giant.damage) + Number(param1.xml.xml.Order.Units.giant.toNotArmour)) * 3;
                    _damageToArmour = (Number(param1.xml.xml.Order.Units.giant.damage) + Number(param1.xml.xml.Order.Units.giant.toArmour)) * 5;
                    _mass = Number(param1.xml.xml.Order.Units.giant.mass) / 2;
               }
               else if(!team.isEnemy)
               {
                    _maxVelocity = param1.xml.xml.Order.Units.giant.maxVelocity;
                    _damageToNotArmour = Number(param1.xml.xml.Order.Units.giant.damage) + Number(param1.xml.xml.Order.Units.giant.toNotArmour);
                    _damageToArmour = Number(param1.xml.xml.Order.Units.giant.damage) + Number(param1.xml.xml.Order.Units.giant.toArmour);
                    _mass = param1.xml.xml.Order.Units.giant.mass;
               }
               ...
               if(this.enslavedgiantType == "E_LavaGiant" && !this.setupComplete)
               {
                    EnslavedGiant.setItem(_mc,"Lava Bag","","");
                    this.setupComplete = true;
               }
               if(this.enslavedgiantType == "E_IceGiant" && !this.setupComplete)
               {
                    EnslavedGiant.setItem(_mc,"Ice Giant Bag","","");
                    this.setupComplete = true;
               }
               if(this.enslavedgiantType == "E_Giant_1" && !this.setupComplete)
               {
                    blueColor = true;
                    EnslavedGiant.setItem(_mc,"","","");
                    this.setupComplete = true;
               }
               if(this.enslavedgiantType == "E_Giant_2" && !this.setupComplete)
               {
                    redColor = true;
                    EnslavedGiant.setItem(_mc,"","","");
                    this.setupComplete = true;
               }
               if(this.enslavedgiantType != "")
               {
                    isCustomUnit = true;
               }
               ...
               {
                    if(this.enslavedgiantType == "E_LavaGiant")
                    {
                         param1.projectileManager.initNuke(px,py,this,90,0.5,850);
                    }
                    isDead = true;
               ...
               if(this.enslavedgiantType == "E_LavaGiant")
               {
                    EnslavedGiant.setItem(mc,"Lava Bag","","");
               }
               else if(this.enslavedgiantType == "E_Giant_1")
               {
                    EnslavedGiant.setItem(mc,"","","");
               }
               else if(this.enslavedgiantType == "E_Giant_2")
               {
                    EnslavedGiant.setItem(mc,"","","");
               }
               else if(this.enslavedgiantType == "E_IceGiant")
               {
                    EnslavedGiant.setItem(_mc,"Ice Giant Bag","","");
               }
               else
          ...
          {
               if(this.enslavedgiantType == "Default")
               {
          ...
          {
               if(this.enslavedgiantType == "E_Giant_2")
               {
                    return 230;
               }
               if(this.enslavedgiantType == "E_Giant_1")
               {
                    return 10;
               }
          ...
          {
               if(this.inBlock || this.enslavedgiantType == "Default" || this.enslavedgiantType == "E_Giant_1" || this.enslavedgiantType == "E_Giant_2")
               {
