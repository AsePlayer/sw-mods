package com.brockw.stickwar.engine.units
{
     ...
     public class Wingidon extends RangedUnit
     {
          ...
          override public function init(param1:StickWar) : void
          {
               ...
               this.loadDamage(param1.xml.xml.Chaos.Units.wingidon);
               _mc.stop();
               _mc.width *= _scale;
               _mc.height *= _scale;
               _hitBoxWidth = 25;
          ...
          override public function update(param1:StickWar) : void
          {
               ...
               if(!isUC)
               {
                    _maxVelocity = param1.xml.xml.Chaos.Units.wingidon.maxVelocity;
               }
               if(isUC)
               {
                    _maxVelocity = param1.xml.xml.Chaos.Units.wingidon.maxVelocity * 1.2;
                    _damageToNotArmour = (Number(param1.xml.xml.Chaos.Units.wingidon.damage) + Number(param1.xml.xml.Chaos.Units.wingidon.toNotArmour)) * 6.5;
                    _damageToArmour = (Number(param1.xml.xml.Chaos.Units.wingidon.damage) + Number(param1.xml.xml.Chaos.Units.wingidon.toArmour)) * 4.4;
                    _mass = Number(param1.xml.xml.Chaos.Units.wingidon.mass) / 2;
               }
               else if(!team.isEnemy)
               {
                    _maxVelocity = param1.xml.xml.Chaos.Units.wingidon.maxVelocity;
                    _damageToNotArmour = Number(param1.xml.xml.Chaos.Units.wingidon.damage) + Number(param1.xml.xml.Chaos.Units.wingidon.toNotArmour);
                    _damageToArmour = Number(param1.xml.xml.Chaos.Units.wingidon.damage) + Number(param1.xml.xml.Chaos.Units.wingidon.toArmour);
                    _mass = param1.xml.xml.Chaos.Units.wingidon.mass;
               }
               ...
               if(this.wingidonType == "VampClipsor" && !this.setupComplete)
               {
                    LightVampRegen = true;
                    LightVampCure = true;
                    Wingidon.setItem(mc,"","Vamp Helmet","Purple Sleeve");
                    maxHealth = 400;
                    health = 400;
                    maxHealth = maxHealth;
                    healthBar.totalHealth = maxHealth;
               ...
               if(this.wingidonType == "VoltWing" && !this.setupComplete)
               {
                    maxHealthIncrease = true;
                    Wingidon.setItem(mc,"","Volt Helmet","Volt Quiver");
                    maxHealth = 400;
                    health = 400;
                    maxHealth = maxHealth;
                    healthBar.totalHealth = maxHealth;
               ...
               if(this.wingidonType == "SuperWing_5" && !this.setupComplete)
               {
                    Wingidon.setItem(mc,"","Demon Helmet","");
                    maxHealth = 700;
                    health = 700;
                    maxHealth = maxHealth;
                    healthBar.totalHealth = maxHealth;
               ...
               {
                    if(this.wingidonType == "SuperWing_3")
                    {
                         _maximumRange = 1100;
                    }
                    else if(isUC)
                    ...
                    else if(this.wingidonType == "SuperWing_2")
                    {
                         _maximumRange = 950;
                    }
                    ...
                    else if(this.wingidonType == "SuperWing_5")
                    {
                         _maximumRange = 850;
                    }
                    else if(this.isCastleArcher)
                    {
                         this.wingidonGlow.color = 16777215;
                         this.wingidonGlow.blurX = 5;
                         this.wingidonGlow.blurY = 5;
                         this.mc.filters = [this.wingidonGlow];
                         redColor = true;
               ...
               else if(this.wingidonType == "VampClipsor")
               {
                    Wingidon.setItem(mc,"","Vamp Helmet","Purple Sleeve");
               }
               else if(this.wingidonType == "VoltWing")
               {
                    Wingidon.setItem(mc,"","Volt Helmet","Volt Quiver");
               }
               else
          ...
          override public function get damageToArmour() : Number
          {
               ...
               if(this.wingidonType == "SuperWing_5")
               {
                    return 25;
               }
          ...
          {
               if(this.inBlock || this.wingidonType == "SuperWing_1")
               {
