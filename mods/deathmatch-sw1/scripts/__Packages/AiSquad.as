class AiSquad
{
     ...
     var DM_ARCHER_TIME = 1500;
     var DM_MINER_TIME = 1000;
     var DM_SWORDMAN_TIME = 1000;
     var DM_SPARTAN_TIME = 3000;
     var DM_WIZARD_TIME = 3000;
     var DM_GIANT_TIME = 7000;
     var GARRISON = 1;
     var DEFEND = 2;
     var ATTACK = 3;
     ...
     function AiSquad(castle, statue, statueHitArea, friends, oponents, baseX, baseY, name, game, technology)
     {
          ...
          if(this.teamName == "red")
          {
               ...
               _loc12_ = new Color(castle);
               _loc12_.setTint(100,0,0,60);
          }
          ...
          var _loc13_ = 1 + technology.getCastleHealthLevel() / 4;
          statue._xscale *= _loc13_ * 80 / 100;
          statue._yscale = _loc13_ * 80;
          this.actionTime = 0;
     ...
     function garrisonUnits()
     {
          ...
          {
               _loc3_ = this.garrisonQueue.pop();
               _loc3_.setIsGarrisoned(true);
               this.numGarrisoned++;
               _loc3_.setIsQueued(false);
     ...
     function addArcher()
     {
          var _loc2_ = new Archer(this.baseX,this.game.getTOP() + (this.game.getTOP() - this.game.getBOTTOM()) * Math.random(),this,this.archerGroup.getMen(),this.game);
          ...
          this.archerGroup.addMan(_loc3_);
          this.addMan(_loc2_);
          this.archerGroup.addToFormation(_loc3_);
          return _loc2_;
     }
     ...
     function addSpartan()
     {
          var _loc2_ = new Spartan(this.baseX,this.game.getTOP() + (this.game.getTOP() - this.game.getBOTTOM()) * Math.random(),this,this.spartanGroup.getMen(),this.game);
          var _loc3_ = {instance:_loc2_,state:0,lastStateChange:this.game.getGameTime(),target:undefined,targetAquireTime:0,targetRange:Infinity,row:undefined,col:undefined,formation:Array};
          this.spartanGroup.addMan(_loc3_);
          this.addMan(_loc2_);
          this.spartanGroup.addToFormation(_loc3_);
          return _loc2_;
     }
     function addGiant()
     {
     ...
     {
          var _loc3_ = formation.length - 1;
          if(formation[_loc3_].length >= 5)
          {
               var _loc4_ = new Array();
               formation.push(_loc4_);
               _loc3_ += 1;
          }
          formation[_loc3_].push(man);
          man.row = _loc3_;
          man.col = formation[_loc3_].length - 1;
     ...
     function filterDown(formation, line, deletedIndex)
     {
          if(line < formation.length - 1)
          {
               ...
               var _loc5_ = formation[line + 1].shift();
               var _loc6_ = Array();
               var _loc7_ = false;
               for(var _loc8_ in formation[line])
               {
                    if(_loc8_ == deletedIndex)
                    {
                         _loc6_.push(_loc5_);
                         _loc7_ = true;
                    }
                    _loc6_.push(formation[line][_loc8_]);
               }
               if(_loc7_ == false)
               {
                    _loc6_.push(_loc5_);
               }
               formation[line] = _loc6_;
          ...
          {
               var _loc9_ = 0;
               while(_loc9_ < formation[line].length)
               {
                    formation[line][_loc9_].row = line;
                    formation[line][_loc9_].col = _loc9_;
                    _loc9_ += 1;
               }
               line += 1;
          }
     ...
     {
          var _loc3_ = undefined;
          var _loc4_ = man.formation;
          _loc4_[man.row].splice(man.col,1);
          this.filterDown(_loc4_,man.row,man.col);
          if(_loc4_[_loc4_.length - 1].length == 0)
          {
               _loc4_.pop();
          }
     }
     function removeManFromFormation(man, simUnits)
     {
          ...
          for(_loc4_ in simUnits)
          {
               if(simUnits[_loc4_].instance == man)
               {
                    this.removeFromFormation(simUnits[_loc4_]);
               }
          }
     }
     function addMan(man)
     ...
     {
          var _loc4_ = undefined;
          for(_loc4_ in this.men)
          {
               if(this.men[_loc4_] == man)
               {
                    this.men.splice(_loc4_,1);
               }
          }
          for(_loc4_ in simUnits)
          {
     ...
     function setGroupMode(newMode, group)
     {
          ...
          if(newMode != this.GARRISON)
          {
               var _loc4_ = this.groups[group - 1].getMen();
               for(var _loc5_ in _loc4_)
               {
                    _loc4_[_loc5_].instance.setIsGarrisoned(false);
                    _loc4_[_loc5_].instance.setIsQueued(false);
               }
          }
     }
     function getGroupMode(group)
     {
          ...
          {
               var _loc3_ = 0;
               while(_loc3_ < this.groups.length - 1)
               {
                    if(this.groups[_loc3_].getStance() != this.groups[_loc3_ + 1].getStance())
                    {
                         return 4;
                    }
                    _loc3_ += 1;
     ...
     function minerGiveGold(g)
     {
          ...
          var _loc3_ = "goldAdded" + this.goldAddedNum;
          this.game.screen.attachMovie("goldAdded",_loc3_,10000000000006 + this.goldAddedNum);
          this.game.screen[_loc3_]._x = this.baseX + 50;
          this.game.screen[_loc3_]._y = 300;
          this.game.screen[_loc3_].gold.gold.text = "+ " + int(g);
          this.giveGold(g);
     }
     function giveGold(g)
     {
     ...
     function addMinerInQueue()
     {
          if(this.technology.gold >= this.MINER_COST and this.canAddMan())
          {
               ...
               if(_root.campaignData.getLevel() == 1)
               {
                    this.queue.push("addMiner",this.DM_MINER_TIME);
               }
               else if(_root.campaignData.getLevel() == 3)
               {
                    this.queue.push("addMiner",this.DM_MINER_TIME);
               }
               else
     ...
     function addArcherInQueue()
     {
          if(this.technology.gold >= this.ARCHER_COST and this.canAddMan())
          {
               ...
               if(_root.campaignData.getLevel() == 1)
               {
                    this.queue.push("addArcher",this.DM_ARCHER_TIME);
               }
               else if(_root.campaignData.getLevel() == 3)
               {
                    this.queue.push("addArcher",this.DM_ARCHER_TIME);
     ...
     function addSpartanInQueue()
     {
          if(this.technology.gold >= this.SPARTAN_COST and this.canAddMan())
          {
               ...
               if(_root.campaignData.getLevel() == 1)
               {
                    this.queue.push("addSpartan",this.DM_SPARTAN_TIME);
               }
               else if(_root.campaignData.getLevel() == 3)
               {
                    this.queue.push("addSpartan",this.DM_SPARTAN_TIME);
     ...
     function addSwordManInQueue()
     {
          if(this.technology.gold >= this.SWORD_MAN_COST and this.canAddMan())
          {
               ...
               if(_root.campaignData.getLevel() == 1)
               {
                    this.queue.push("addSwordMan",this.DM_SWORDMAN_TIME);
               }
               else if(_root.campaignData.getLevel() == 3)
               {
                    this.queue.push("addSwordMan",this.DM_SWORDMAN_TIME);
     ...
     function addGiantInQueue()
     {
          if(this.technology.gold >= this.GIANT_COST and this.canAddMan())
          {
               ...
               if(_root.campaignData.getLevel() == 1)
               {
                    this.queue.push("addGiant",this.DM_GIANT_TIME);
               }
               else if(_root.campaignData.getLevel() == 3)
               {
                    this.queue.push("addGiant",this.DM_GIANT_TIME);
     ...
     function addWizardInQueue()
     {
          if(this.technology.gold >= this.WIZARD_COST and this.canAddMan())
          {
               ...
               if(_root.campaignData.getLevel() == 1)
               {
                    this.queue.push("addWizard",this.DM_WIZARD_TIME);
               }
               else if(_root.campaignData.getLevel() == 3)
               {
                    this.queue.push("addWizard",this.DM_WIZARD_TIME);
     ...
     function getForwardMan(exclude)
     {
          ...
          while(_loc5_ < 6)
          {
               var _loc6_ = this.groups[_loc5_].getFormation()[0][0].instance;
               if(_loc6_ != undefined && _loc5_ != exclude)
               {
                    if(_loc6_.getX() * this.getTeamDirection() > _loc4_ * this.getTeamDirection())
                    {
                         _loc4_ = _loc6_.getX();
                         _loc3_ = _loc6_;
                    }
     ...
     function enableGlow()
     {
          for(var _loc2_ in this.men)
          {
               if(this.men[_loc2_].getIsAlive())
               {
                    this.men[_loc2_].enableGlow();
               }
          }
          var _loc3_ = this.castleArcherGroup.getMen();
          for(_loc2_ in _loc3_)
          {
               if(_loc3_[_loc2_].getIsAlive())
               {
                    _loc3_[_loc2_].enableGlow();
               }
          }
     }
