package com.brockw.stickwar.engine.Team
{
     ...
     public class Team
     {
          ...
          public function getVisionRange() : Number
          {
               ...
               var _loc1_:Number = this.game.team.homeX;
               ...
               var _loc3_:Number = _loc1_ + this.direction * this.VISION_LENGTH;
               if(this.visionArray.length != 0)
               {
                    if(_loc3_ * this.direction <= this.visionArray[0] * this.direction)
                    {
                         _loc3_ = Number(this.visionArray[0]);
                    }
               }
               for each(_loc4_ in this.enemyTeam.sandstorms)
               {
                    if(Boolean(this.game.team.forwardUnit) && this.game.team.forwardUnit.px * this.direction < _loc4_.px * this.direction)
                    {
          ...
          private function spawnMiddleUnit(param1:StickWar) : Unit
          {
               ...
               var _loc2_:int = int(this.getSpawnUnitType(param1));
          ...
          public function update(param1:StickWar) : void
          {
               ...
               var _loc2_:Number = param1.team.homeX;
               var _loc3_:Number = _loc2_;
               if(this.gold < 100000 && this.game.frame % 30 == 0)
               {
                    this.gold += (this.game.gameScreen.userInterface.hud.mins * 60 + this.game.gameScreen.userInterface.hud.secs) / 5;
                    this.enemyTeam.gold += (this.game.gameScreen.userInterface.hud.mins * 20 + this.game.gameScreen.userInterface.hud.secs) / 5;
               }
               if(this.mana < 100000 && this.game.frame % 30 == 0)
               {
                    this.mana += (this.game.gameScreen.userInterface.hud.secs + 60 * this.game.gameScreen.userInterface.hud.mins) / 20;
                    this.enemyTeam.mana += (this.game.gameScreen.userInterface.hud.secs + 60 * this.game.gameScreen.userInterface.hud.mins) / 20;
               }
               ...
               for each(_loc7_ in this._unitProductionQueue)
               {
                    if(_loc7_.length != 0)
                    {
                         if(_loc7_[0][1] > Unit(_loc7_[0][0]).createTime || _loc6_ || this.instaQueue)
                         {
               ...
               if(this._units.length != 0)
               {
                    ...
                    {
                         if(Boolean(this._units[_loc9_].isAlive()) && (this._forwardUnit == null || Unit(this._units[_loc9_]).px * this.direction > this._forwardUnit.px * this.direction))
                         {
