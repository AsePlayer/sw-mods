package com.brockw.stickwar.engine.units
{
     ...
     public class Spearton extends com.brockw.stickwar.engine.units.Unit
     {
          ...
          override public function update(param1:StickWar) : void
          {
               ...
               updateCommon(param1);
               if(this.speartonType == "SpeartonLeader" && !this.setupComplete)
               {
                    WEAPON_REACH = 60;
                    maxHealth = 100;
                    health = 100;
               ...
               if(this.speartonType == "EnchanceRebelSpearton" && !this.setupComplete)
               {
                    maxHealth = 700;
                    health = 700;
                    maxHealth = maxHealth;
               ...
               if(this.speartonType == "CAP" && !this.setupComplete)
               {
                    maxHealth = 800;
                    health = 800;
                    maxHealth = maxHealth;
               ...
               if(this.speartonType == "Mitchell" && !this.setupComplete)
               {
                    maxHealth = 1200;
                    health = 1200;
                    maxHealth = maxHealth;
               ...
               if(this.speartonType == "SpeartonLeader")
               {
                    Spearton.setItem(_speartonMc(mc),"","Wing Helmet","");
               }
               else if(this.speartonType == "EnchanceRebelSpearton")
               {
                    Spearton.setItem(_speartonMc(mc),team.loadout.getItem(this.type,MarketItem.T_WEAPON),"Hedgehog Helmet",team.loadout.getItem(this.type,MarketItem.T_MISC));
               }
               else if(this.speartonType == "CAP")
               {
                    Spearton.setItem(_speartonMc(mc),"Leaf Spear","Gladiator Helmet","Scorpion Shield");
               }
               else if(this.speartonType == "Mitchell")
               {
                    Spearton.setItem(_speartonMc(mc),"Ice Spear","Ice Speartan Helmet","Ice Shield");
               }
               else
               {
