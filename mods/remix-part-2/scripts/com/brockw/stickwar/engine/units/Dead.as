package com.brockw.stickwar.engine.units
{
     ...
     public class Dead extends RangedUnit
     {
          ...
          override public function update(param1:StickWar) : void
          {
               ...
               super.update(param1);
               updateCommon(param1);
               if(this.deadType == "StoneDead" && !this.setupComplete)
               {
                    maxHealth = 2500;
                    health = 2500;
                    maxHealth = maxHealth;
                    ...
                    _scale = 2;
                    this.poisonDamageAmount = 12;
                    this._maximumRange = 500;
                    _maxVelocity = 1;
                    this.setupComplete = true;
               }
               if(this.deadType == "StoneDeadCracked" && !this.setupComplete)
               {
                    maxHealth = 1500;
                    health = 1500;
                    maxHealth = maxHealth;
                    healthBar.totalHealth = maxHealth;
                    _scale = 1.4;
                    this.poisonDamageAmount = 10;
                    this._maximumRange = 600;
                    _maxVelocity = 1;
                    this.setupComplete = true;
               }
               if(this.deadType == "StoneDeadDown" && !this.setupComplete)
               {
                    maxHealth = 750;
                    health = 750;
                    maxHealth = maxHealth;
                    healthBar.totalHealth = maxHealth;
                    _scale = 1;
                    this.poisonDamageAmount = 8;
                    this._maximumRange = 700;
                    _maxVelocity = 1;
                    this.setupComplete = true;
               }
               if(this.deadType == "MeatShield" && !this.setupComplete)
               {
                    _maximumRange = 25;
                    maxHealth = 400;
                    health = 400;
               ...
               {
                    if(this.deadType == "StoneDead")
                    {
                         param1.projectileManager.initTowerSpawn(this.px,this.py,param1.team,0.6);
                         this.clusterLad = team.game.unitFactory.getUnit(com.brockw.stickwar.engine.units.Unit.U_DEAD);
                         team.spawn(this.clusterLad,team.game);
                         this.clusterLad.px = this.px;
                         this.clusterLad.py = this.py;
                         this.clusterLad.deadType = "StoneDeadCracked";
                         param1.projectileManager.initTowerSpawn(this.px,this.py,param1.team,0.6);
                         this.clusterLad = team.game.unitFactory.getUnit(com.brockw.stickwar.engine.units.Unit.U_DEAD);
                         team.spawn(this.clusterLad,team.game);
                         this.clusterLad.px = this.px;
                    ...
                    else if(this.deadType == "StoneDeadCracked")
                    {
                         param1.projectileManager.initTowerSpawn(this.px,this.py,param1.team,0.6);
                         this.clusterLad = team.game.unitFactory.getUnit(com.brockw.stickwar.engine.units.Unit.U_DEAD);
                         team.spawn(this.clusterLad,team.game);
                         this.clusterLad.px = this.px;
                         this.clusterLad.py = this.py;
                         this.clusterLad.deadType = "StoneDeadDown";
                         param1.projectileManager.initTowerSpawn(this.px,this.py,param1.team,0.6);
                         this.clusterLad = team.game.unitFactory.getUnit(com.brockw.stickwar.engine.units.Unit.U_DEAD);
                         team.spawn(this.clusterLad,team.game);
                         this.clusterLad.px = this.px;
               ...
               if(this.isCastleArcher)
               {
                    com.brockw.stickwar.engine.units.Dead.setItem(mc,"Default","Jason Mask","Default");
               }
               else if(this.deadType == "StoneDead")
               {
                    com.brockw.stickwar.engine.units.Dead.setItem(mc,"Default","Wrapped Helmet","Default");
               }
               else if(this.deadType == "StoneDeadCracked")
               {
                    com.brockw.stickwar.engine.units.Dead.setItem(mc,"Default","Dead Clown","Default");
               }
               else if(this.deadType == "StoneDeadDown")
               {
                    com.brockw.stickwar.engine.units.Dead.setItem(mc,"Default","Roots Dead","Default");
               }
               else if(this.deadType == "MeatShield" && !this.setupComplete)
               {
                    com.brockw.stickwar.engine.units.Dead.setItem(mc,"Default","Wrapped Helmet","Default");
               }
               else
               {
                    com.brockw.stickwar.engine.units.Dead.setItem(mc,team.loadout.getItem(this.type,MarketItem.T_WEAPON),team.loadout.getItem(this.type,MarketItem.T_ARMOR),team.loadout.getItem(this.type,MarketItem.T_MISC));
               }
          }
          ...
          override public function get damageToArmour() : Number
          {
               ...
               if(this.deadType == "StoneDeadCracked")
               {
                    return 65;
               }
               if(this.deadType == "StoneDeadDown")
               {
                    return 45;
               }
               if(this.deadType == "MeatShield")
               {
                    return 0;
               }
          ...
          {
               if(this.inBlock || this.deadType == "MeatShield")
               {
