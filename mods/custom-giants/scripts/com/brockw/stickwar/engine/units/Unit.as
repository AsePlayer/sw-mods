package com.brockw.stickwar.engine.units
{
     ...
     public class Unit extends Entity
     {
          ...
          public function Unit(param1:StickWar)
          {
               ...
               this.towerSpawnGlow = new GlowFilter(65535,1,36,18,2.64,_loc7_,false,false);
          ...
          public function initBase() : void
          {
               ...
               {
                    if(_loc1_.name == "stone")
                    {
                         this.isStoneable = true;
                    }
                    if(_loc1_.name == "fireDeath")
                    {
                         this.isFirable = true;
                    }
               }
          ...
          protected function updateCommon(param1:StickWar) : void
          {
               ...
               if(this.isAlive() && param1.frame % 30 == 0 && this._poisonDamage > 0)
               {
                    if(this.poisonRegen)
                    {
          ...
          private function drawSelected(param1:int, param2:Number) : void
          {
               if(this.shadowSprite == null)
               {
                    return;
               ...
               this.shadowSprite.graphics.clear();
               this.shadowSprite.graphics.lineStyle(1,param1,1);
               this.shadowSprite.graphics.beginFill(param1,0.3);
               this.shadowSprite.graphics.drawEllipse(-40,this.flyingHeight - 4.5,80,15);
          }
          
          public function poison(param1:Number) : void
          {
               if(!this.isAlive())
               {
                    return;
          ...
          protected function updateMotion(param1:StickWar) : void
          {
               ...
               if(!this.isGarrisoned)
               {
                    if(px + this._dx <= param1.map.screenWidth + _loc3_ && this._dx < 0)
                    {
                         this._dx = 0;
                    }
                    if(px + this._dx >= param1.background.mapLength - param1.map.screenWidth - _loc3_ && this._dx > 0)
                    {
                         this._dx = 0;
                    }
                    if(!(this.ai.currentCommand is HoldCommand))
                    {
                         if(px + this._dx <= param1.map.screenWidth + _loc3_)
                         {
                              this.walk(1,0,1);
                         }
                         if(px + this._dx >= param1.background.mapLength - param1.map.screenWidth - _loc3_)
                         {
                              this.walk(-1,0,-1);
                         }
                    }
               ...
               px += this._dx;
               py += this._dy;
               for each(_loc2_ in this.team.enemyTeam.walls)
               {
                    if(px < _loc2_.px + 7.5 && px > _loc2_.px - 7.5)
                    {
                         px = _loc2_.px - 7.5 * this.team.direction;
                    }
               }
               if(px < 0 + 7.5)
               {
          ...
          public function stun(param1:int) : void
          {
               if(!this.isAlive())
               {
                    return;
               ...
               if(param1 > 0)
               {
                    this._dz = -com.brockw.stickwar.engine.units.Unit.stunUpForce;
          ...
          public function checkForHitPointArrow(param1:Point, param2:Number, param3:com.brockw.stickwar.engine.units.Unit) : Boolean
          {
               ...
               var _loc4_:int = Util.sgn(param3.px - px);
               param1 = this.globalToLocal(param1);
               if(param1.x > -pwidth && param1.x < pwidth && param1.y > -pheight && param1.y < 0 && Math.abs(param2 - this.py) < 150)
               {
                    return true;
               }
               return false;
          ...
          override public function damage(param1:int, param2:int, param3:Entity, param4:Number = 1) : void
          {
               ...
               if(this.isTargetable() && !this.isDualing)
               {
                    ...
                    _loc5_ /= this.team.healthModifier;
                    _loc5_ *= this.team.enemyTeam.damageModifier;
                    this._health -= _loc5_;
                    if(this._health <= 0)
                    {
                         this.arrowDeath = false;
