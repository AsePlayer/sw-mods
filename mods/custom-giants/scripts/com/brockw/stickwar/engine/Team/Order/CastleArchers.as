package com.brockw.stickwar.engine.Team.Order
{
     import com.brockw.stickwar.engine.Ai.ArcherAi;
     import com.brockw.stickwar.engine.Ai.command.HoldCommand;
     import com.brockw.stickwar.engine.StickWar;
     import com.brockw.stickwar.engine.Team.CastleDefence;
     import com.brockw.stickwar.engine.Team.Team;
     import com.brockw.stickwar.engine.Team.Tech;
     import com.brockw.stickwar.engine.units.Archer;
     
     public class CastleArchers extends CastleDefence
     {
           
          
          public function CastleArchers(param1:StickWar, param2:Team)
          {
               super(param1,param2);
               units = [];
          }
          
          override public function update(param1:StickWar) : void
          {
               var _loc2_:int = 0;
               var _loc3_:int = 0;
               if(team.tech.isResearched(Tech.CASTLE_ARCHER_5))
               {
                    _loc3_ = 5;
               }
               else if(team.tech.isResearched(Tech.CASTLE_ARCHER_4))
               {
                    _loc3_ = 6;
               }
               else if(team.tech.isResearched(Tech.CASTLE_ARCHER_3))
               {
                    _loc3_ = 5;
               }
               else if(team.tech.isResearched(Tech.CASTLE_ARCHER_2))
               {
                    _loc3_ = 4;
               }
               else if(team.tech.isResearched(Tech.CASTLE_ARCHER_1))
               {
                    _loc3_ = 2;
               }
               if(units.length < _loc3_)
               {
                    this.addUnit();
               }
               _loc2_ = 0;
               while(_loc2_ < units.length)
               {
                    units[_loc2_].faceDirection(team.direction);
                    _loc2_++;
               }
               super.update(param1);
          }
          
          override public function addUnit() : void
          {
               var _loc1_:Archer = null;
               _loc1_ = new Archer(game);
               _loc1_.ai = new ArcherAi(_loc1_);
               _loc1_.team = team;
               _loc1_.isCastleArcher = true;
               _loc1_.init(game);
               _loc1_.flyingHeight = 450;
               _loc1_.pz = -_loc1_.flyingHeight;
               _loc1_.ai.init();
               if(this.team.type == Team.T_GOOD)
               {
                    _loc1_.py = game.map.height / 1.8 * units.length / game.xml.xml.Order.Tech.castleArchers.num;
                    _loc1_.y = _loc1_.py;
                    _loc1_.px = team.homeX + team.direction * 175 - team.direction * units.length * 6.6;
               }
               else
               {
                    _loc1_.py = -game.map.height / 5 + 3 * game.map.height / 4 * units.length / game.xml.xml.Order.Tech.castleArchers.num;
                    _loc1_.y = 0;
                    _loc1_.px = team.homeX + team.direction * 110 - team.direction * units.length * 8;
               }
               _loc1_.x = _loc1_.px;
               var _loc2_:HoldCommand = new HoldCommand(game);
               _loc1_.ai.setCommand(game,_loc2_);
               units.push(_loc1_);
               game.battlefield.addChild(_loc1_);
          }
     }
}
