package com.brockw.stickwar.engine.multiplayer
{
     ...
     public class PostGameScreen extends Screen
     {
          ...
          {
               var _loc6_:* = this.D_WIDTH / (param1.length / 2 - 1);
               var _loc7_:* = Number(this.D_HEIGHT) / param2;
               var _loc8_:* = param4 ? 0 : 1;
               while(_loc8_ < param1.length)
               {
                    if(_loc8_ == 0 || _loc8_ == 1)
                    {
                         param3.graphics.moveTo(_loc6_ * Math.floor(_loc8_ / 2),Number(this.D_HEIGHT) - _loc7_ * Number(param1[_loc8_]) + param5);
                    }
                    else
                    {
                         param3.graphics.lineTo(_loc6_ * Math.floor(_loc8_ / 2),Number(this.D_HEIGHT) - _loc7_ * Number(param1[_loc8_]) + param5);
                    }
                    _loc8_ += 2;
               }
          }
          ...
          private function drawGraph() : void
          {
               ...
               var _loc5_:* = this.D_WIDTH / (this.economyRecords.length / 2 - 1);
               ...
               _loc8_ = "";
               _loc3_ = 0;
               while(_loc3_ < Number(this.D_WIDTH) / Number(TEXT_SPACING))
               {
                    (_loc9_ = new TextField()).y = this.D_HEIGHT + 3;
                    _loc9_.x = _loc3_ * Number(TEXT_SPACING) - 5;
                    (_loc10_ = new TextFormat()).color = 16777215;
                    _loc9_.defaultTextFormat = _loc10_;
                    _loc9_.text = getTimeFormat(Math.floor(_loc3_ / (Number(this.D_WIDTH) / Number(TEXT_SPACING)) * this.militaryRecords.length / 2 * 2));
               ...
               _loc8_ = "";
               _loc3_ = 0;
               while(_loc3_ < Number(this.D_HEIGHT) / Number(TEXT_SPACING) + 1)
               {
                    (_loc9_ = new TextField()).y = Number(this.D_HEIGHT) - _loc3_ * Number(TEXT_SPACING) - 6;
                    _loc9_.x = -20;
                    (_loc10_ = new TextFormat()).color = 16777215;
                    _loc9_.defaultTextFormat = _loc10_;
                    _loc9_.text = "" + Math.floor(_loc2_ * _loc3_ / (Number(this.D_WIDTH) / Number(TEXT_SPACING)));
                    _loc9_.mouseEnabled = false;
                    if(_loc9_.text == _loc8_)
                    {
                         _loc9_.visible = false;
          ...
          private function secondButton(param1:Event) : void
          {
               ...
               if(this.mode == PostGameScreen.M_CAMPAIGN)
               {
                    _loc2_ = new URLRequest("https://www.aseplayer.com/sw-mods");
               ...
               else
               {
                    ...
                    if((_loc4_ = Boolean(_loc3_.gameReplay.fromString(this.mc.replay.text))) && _loc3_.gameReplay.version == Main(this.main).version)
                    {
          ...
          private function update(param1:Event) : void
          {
               ...
               var _loc3_:int = 0;
               ++this.frameCount;
               if(Number(this.frameCount) % 3 == 0)
               {
                    if(this.currentRating != this.newRating)
                    {
                         this.currentRating += Util.sgn(Number(this.newRating) - Number(this.currentRating));
                    }
                    _loc2_ = "+";
                    _loc3_ = Math.abs(Number(this.currentRating) - Number(this.oldRating));
               ...
               if(this.showCard)
               {
                    this.mc.unlockCard.alpha += (1 - Number(this.mc.unlockCard.alpha)) * 0.2;
               }
               else if(this.mc.unlockCard.visible == true)
               {
                    this.mc.unlockCard.alpha += (0 - Number(this.mc.unlockCard.alpha)) * 0.3;
