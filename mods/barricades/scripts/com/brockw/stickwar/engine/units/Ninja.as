package com.brockw.stickwar.engine.units
{
     ...
     public class Ninja extends com.brockw.stickwar.engine.units.Unit
     {
          ...
          internal var ShinobiDamage:Array;
          ...
          public function stealth() : void
          {
               if(techTeam.tech.isResearched(Tech.CLOAK))
               {
                    ...
                    {
                         if(this.ninjaType == "ArmoredNinja")
                         {
                              this.team.game.projectileManager.initTowerSpawn(px,py,this.team,0.6);
                              this.px += 500;
                         }
                         if(this.ninjaType == "ArmoredNinja_2")
                         {
                              this.team.game.projectileManager.initTowerSpawn(px,py,this.team,0.6);
                              this.px -= 500;
                         }
                         if(this.ninjaType == "ChaosNinja")
                         {
                              this.team.game.projectileManager.initTowerSpawn(px,py,this.team,0.6);
                              this.px += 500;
                         }
                         if(this.ninjaType == "ChaosNinja_2")
                         {
                              this.team.game.projectileManager.initTowerSpawn(px,py,this.team,0.6);
                              this.px -= 500;
                         }
          ...
          override protected function checkForHit() : Boolean
          {
               ...
               if(_loc1_.checkForHitPoint(_loc3_,_loc1_))
               {
                    ...
                    if(!this.dontStealth)
                    {
                         ...
                         if(this.ninjaType == "LeafNinja")
                         {
                              this.team.game.projectileManager.initThorn(_loc1_.px,_loc1_.py,this,_loc1_,this.team,100,200);
                              this.team.game.soundManager.playSound("thornSound",_loc1_.px,_loc1_.py);
                         }
          ...
          override public function update(param1:StickWar) : void
          {
               ...
               if(!isUC)
               {
                    this.normalVelocity = param1.xml.xml.Order.Units.ninja.maxVelocity;
                    this.maxStacks = param1.xml.xml.Order.Units.ninja.fury.stacks;
                    this.stackDamage = param1.xml.xml.Order.Units.ninja.fury.bonus;
               }
               if(isUC)
               {
                    this.maxStacks = param1.xml.xml.Order.Units.ninja.fury.stacks * 2;
                    this.stackDamage = param1.xml.xml.Order.Units.ninja.fury.bonus * 2;
                    this.normalVelocity = param1.xml.xml.Order.Units.ninja.maxVelocity * 1.2;
                    _damageToNotArmour = (Number(param1.xml.xml.Order.Units.ninja.damage) + Number(param1.xml.xml.Order.Units.ninja.toArmour)) * 2;
                    _damageToArmour = (Number(param1.xml.xml.Order.Units.ninja.damage) + Number(param1.xml.xml.Order.Units.ninja.toArmour)) * 2;
                    _mass = Number(param1.xml.xml.Order.Units.ninja.mass) / 2;
               }
               ...
               if(this.ninjaType == "ArmoredNinja" && !this.setupComplete)
               {
                    noStone = true;
                    nonTeleportable = true;
                    Ninja.setItem(_mc,"Ripp Blade","Ripp Helmet","");
                    maxHealth = 1300;
                    health = 1300;
                    maxHealth = maxHealth;
                    healthBar.totalHealth = maxHealth;
               ...
               if(this.ninjaType == "ArmoredNinja_2" && !this.setupComplete)
               {
                    noStone = true;
                    nonTeleportable = true;
                    Ninja.setItem(_mc,"Ripp Blade","Ripp Helmet","");
               ...
               if(this.ninjaType == "ChaosNinja" && !this.setupComplete)
               {
                    noStone = true;
                    nonTeleportable = true;
                    Ninja.setItem(_mc,"","Chaos Helmet","");
                    maxHealth = 600;
                    health = 600;
                    maxHealth = maxHealth;
                    healthBar.totalHealth = maxHealth;
                    _scale = 0.9;
                    this._stealthSpellTimer = new com.brockw.stickwar.engine.units.SpellCooldown(500,300,param1.xml.xml.Order.Units.ninja.stealthMana);
                    this.setupComplete = true;
               }
               if(this.ninjaType == "ChaosNinja_2" && !this.setupComplete)
               {
                    noStone = true;
                    nonTeleportable = true;
                    Ninja.setItem(_mc,"Knifed Pole","Chaos Helmet","Ninja Claws");
                    maxHealth = 600;
                    health = 600;
                    maxHealth = maxHealth;
                    healthBar.totalHealth = maxHealth;
                    _scale = 0.9;
                    this.shieldwallDamageReduction = 0.36;
                    this.setupComplete = true;
               }
               if(this.ninjaType == "VoltNinja" && !this.setupComplete)
               {
                    maxHealthIncrease = true;
                    Ninja.setItem(_mc,"Volt Staff","Volt Helmet","Volt Katana");
                    maxHealth = 300;
                    health = 300;
                    maxHealth = maxHealth;
                    healthBar.totalHealth = maxHealth;
               ...
               if(this.ninjaType == "IceNinja" && !this.setupComplete)
               {
                    slowRegen = true;
                    weakToFire = true;
                    selfFreeze = true;
                    Ninja.setItem(_mc,"Ice Ninja Staff","Ice Ninja Head","");
                    maxHealth = 300;
                    health = 300;
                    maxHealth = maxHealth;
                    healthBar.totalHealth = maxHealth;
               ...
               if(isCustomUnit == true)
               {
                    ...
                    else if(this.ninjaType == "ChaosNinja_2")
                    {
                         this.maxStacks = 10;
                         this.stackDamage = 15;
                    }
                    else if(this.ninjaType == "ArmoredNinja" || this.ninjaType == "ArmoredNinja_2")
                    {
                         this.maxStacks = 15;
                         this.stackDamage = 20;
                         _mass = 35;
               ...
               else if(isDead == false)
               {
                    if(this.ninjaType == "")
                    {
                         param1.projectileManager.initNuke(px,py,this,35,0.4,350);
                    }
                    if(this.ninjaType == "VoltNinja")
                    {
                    ...
                    if(this.ninjaType == "LeafNinja")
                    {
                         param1.projectileManager.initHurricane(px,py,this,0);
                    }
                    isDead = true;
                    if(_isDualing)
                    {
               ...
               else if(this.ninjaType == "IceNinja")
               {
                    Ninja.setItem(_mc,"Ice Ninja Staff","Ice Ninja Head","");
               }
               else if(this.ninjaType == "CrystalNinja")
               ...
               else if(this.ninjaType == "ArmoredNinja")
               {
                    Ninja.setItem(_mc,"Ripp Blade","Ripp Helmet","");
               }
               else if(this.ninjaType == "ArmoredNinja_2")
               {
                    Ninja.setItem(_mc,"Ripp Blade","Ripp Helmet","");
               }
               else if(this.ninjaType == "ChaosNinja")
               {
                    Ninja.setItem(_mc,"","Chaos Helmet","");
               }
               else if(this.ninjaType == "ChaosNinja_2")
               ...
               else if(this.ninjaType == "VoltNinja")
               {
                    Ninja.setItem(_mc,"Volt Staff","Volt Helmet","Volt Katana");
               }
               else
          ...
          {
               if(this.inBlock || this.ninjaType == "ChaosNinja_2")
               {
