package com.brockw.stickwar.engine.units.elementals
{
     ...
     import com.brockw.stickwar.engine.Ai.ChromeAi;
     ...
     import com.brockw.stickwar.engine.units.SpellCooldown;
     ...
     public class ChromeElement extends Unit
     {
          ...
          public var chromeElementType:String;
          ...
          {
               if(techTeam.tech.isResearched(Tech.CHROME_SPLIT_1) && !this.isCopy && this.children.length == 0 && team.game.frame - this.timeSpawnDead > this.CLONE_SPAWN_WAIT && this._splitSpellTimer.spellActivate(team))
               {
          ...
          private function doTheSplit() : void
          {
               ...
               var _loc3_:WaterElement = null;
               var _loc5_:FireElement = null;
               var _loc6_:Archer = null;
               var _loc7_:HurricaneElement = null;
               var _loc2_:AttackMoveCommand = null;
               if(this.chromeElementType == "")
               {
               ...
               if(this.chromeElementType == "Legend")
               {
                    _loc7_ = team.game.unitFactory.getUnit(Unit.U_HURRICANE_ELEMENT);
                    team.spawn(_loc7_,team.game);
                    _loc7_.px = px;
                    _loc7_.py = py + -100;
                    if(_loc7_.py < 0)
                    {
                    ...
                    if(_loc7_.py > team.game.map.height)
                    {
                         _loc7_.py = team.game.map.height;
                    }
                    ...
                    _loc7_.ai.setCommand(team.game,_loc2_);
                    this.children = [_loc7_];
                    _loc3_ = team.game.unitFactory.getUnit(Unit.U_WATER_ELEMENT);
                    team.spawn(_loc3_,team.game);
                    _loc3_.px = px;
                    _loc3_.py = py + -100;
                    if(_loc3_.py < 0)
                    {
                    ...
                    if(_loc3_.py > team.game.map.height)
                    {
                         _loc3_.py = team.game.map.height;
                    }
                    _loc2_ = new AttackMoveCommand(team.game);
                    _loc2_.type = UnitCommand.ATTACK_MOVE;
                    _loc2_.goalX = team.enemyTeam.statue.px;
                    ...
                    _loc2_.realX = team.enemyTeam.statue.px;
                    _loc2_.realY = _loc2_.goalY;
                    _loc3_.ai.setCommand(team.game,_loc2_);
                    this.children = [_loc3_];
                    _loc3_ = team.game.unitFactory.getUnit(Unit.U_WATER_ELEMENT);
                    team.spawn(_loc3_,team.game);
                    _loc3_.px = px;
                    _loc3_.py = py + -100;
                    ...
                    _loc3_.ai.setCommand(team.game,_loc2_);
                    this.children = [_loc3_];
                    _loc6_ = team.game.unitFactory.getUnit(Unit.U_ARCHER);
                    team.spawn(_loc6_,team.game);
                    _loc6_.px = px;
                    _loc6_.py = py + -100;
                    if(_loc6_.py < 0)
                    {
                    ...
                    if(_loc6_.py > team.game.map.height)
                    {
                         _loc6_.py = team.game.map.height;
                    }
                    _loc2_ = new AttackMoveCommand(team.game);
                    _loc2_.type = UnitCommand.ATTACK_MOVE;
                    _loc2_.goalX = team.enemyTeam.statue.px;
                    ...
                    _loc2_.realX = team.enemyTeam.statue.px;
                    _loc2_.realY = _loc2_.goalY;
                    _loc6_.ai.setCommand(team.game,_loc2_);
                    ...
                    _loc6_.ai.setCommand(team.game,_loc2_);
                    this.children = [_loc6_];
                    _loc5_ = team.game.unitFactory.getUnit(Unit.U_FIRE_ELEMENT);
                    team.spawn(_loc5_,team.game);
                    _loc5_.px = px;
                    _loc5_.py = py + -100;
                    if(_loc5_.py < 0)
                    {
                    ...
                    if(_loc5_.py > team.game.map.height)
                    {
                         _loc5_.py = team.game.map.height;
                    }
                    _loc2_ = new AttackMoveCommand(team.game);
                    _loc2_.type = UnitCommand.ATTACK_MOVE;
                    _loc2_.goalX = team.enemyTeam.statue.px;
                    ...
                    _loc2_.realX = team.enemyTeam.statue.px;
                    _loc2_.realY = _loc2_.goalY;
                    _loc5_.ai.setCommand(team.game,_loc2_);
                    this.children = [_loc5_];
               }
          ...
          {
               if(this._convertSpellTimer.spellActivate(team) && param1.type != Unit.U_STATUE && param1.type != Unit.U_WALL)
               {
          ...
          override public function update(param1:StickWar) : void
          {
               ...
               if(team.isEnemy && !enemyBuffed)
               {
                    ...
                    _scale = _scale + Number(team.game.main.campaign.difficultyLevel) * 0.08 - 0.08;
                    enemyBuffed = true;
               }
               if(this.chromeElementType == "Legend" && !this.setupComplete)
               {
                    this.isLegend = true;
                    generalRegen = true;
                    generalRegen2 = true;
                    ...
                    LightVampRegen = true;
                    unitRegen = true;
                    ChromeElement.setItem(_mc,"","White Shell ChromeHead","");
                    maxHealth = 500;
                    health = 500;
                    maxHealth = maxHealth;
                    healthBar.totalHealth = maxHealth;
                    _scale = 0.9;
                    _maxVelocity = 6;
                    this._splitSpellTimer = new SpellCooldown(param1.xml.xml.Elemental.Units.chrome.split.effect,1600,param1.xml.xml.Elemental.Units.chrome.split.mana);
                    this._convertSpellTimer = new SpellCooldown(0,1400,param1.xml.xml.Elemental.Units.chrome.convert.mana);
               ...
               else if(isDead == false)
               {
                    ...
                    this.children = [];
                    if(this.convertedUnit != null && this.convertedUnit.isAlive())
                    {
          ...
          {
               return this.isSplitting || this.isCopy || this.convertedUnit != null;
          }
