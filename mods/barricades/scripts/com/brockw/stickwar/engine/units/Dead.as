package com.brockw.stickwar.engine.units
{
     ...
     public class Dead extends RangedUnit
     {
          ...
          override public function update(param1:StickWar) : void
          {
               ...
               if(isUC)
               {
                    _maxVelocity = param1.xml.xml.Chaos.Units.dead.maxVelocity * 1.2;
                    _damageToNotArmour = (Number(param1.xml.xml.Chaos.Units.dead.damage) + Number(param1.xml.xml.Chaos.Units.dead.toNotArmour)) * 2.2;
                    _damageToArmour = (Number(param1.xml.xml.Chaos.Units.dead.damage) + Number(param1.xml.xml.Chaos.Units.dead.toArmour)) * 4.5;
                    _mass = Number(param1.xml.xml.Chaos.Units.dead.mass) / 2;
                    this.poisonDamageAmount = param1.xml.xml.Chaos.Units.dead.poison.damage * 4;
               }
               else if(!team.isEnemy)
               {
                    _maxVelocity = param1.xml.xml.Chaos.Units.dead.maxVelocity;
                    _damageToNotArmour = Number(param1.xml.xml.Chaos.Units.dead.damage) + Number(param1.xml.xml.Chaos.Units.dead.toNotArmour);
                    _damageToArmour = Number(param1.xml.xml.Chaos.Units.dead.damage) + Number(param1.xml.xml.Chaos.Units.dead.toArmour);
                    _mass = param1.xml.xml.Chaos.Units.dead.mass;
               }
               else if(team.isEnemy && !enemyBuffed && !this.isCastleArcher)
               {
                    _damageToNotArmour *= 1.2;
                    _damageToArmour *= 1.2;
                    health *= 1.3;
                    maxHealth = health;
                    maxHealth = maxHealth;
               ...
               if(this.deadType == "MeleeDead" && !this.setupComplete)
               {
                    LightVampRegen = true;
                    Dead.setItem(mc,"","","");
                    maxHealth = 72;
               ...
               if(this.deadType == "VoltDead" && !this.setupComplete)
               {
                    maxHealthIncrease = true;
                    Dead.setItem(mc,"","Volt Dead Helmet","");
                    maxHealth = 250;
                    health = 250;
                    maxHealth = maxHealth;
                    healthBar.totalHealth = maxHealth;
               ...
               if(isCustomUnit == true)
               {
                    ...
                    else if(this.deadType == "MeleeDead")
                    {
                         _maximumRange = 120;
                         this.poisonDamageAmount = 0;
                    }
               ...
               else if(isDead == false)
               {
                    if(this.deadType == "Deadboss")
                    {
                         param1.projectileManager.initNuke(px,py,this,20,0.2,140);
               ...
               else if(this.deadType == "VoltDead")
               {
                    Dead.setItem(mc,"","Volt Dead Helmet","");
               }
               else if(this.deadType == "SpeedDead")
