package com.brockw.stickwar.engine.Ai
{
     ...
     public class UnitAi
     {
          ...
          public var speartonTargeter:Boolean = false;
          
          public var deadTargeter:Boolean = false;
          
          public var knightTargeter:Boolean = false;
          ...
          public var archerTargeter:Boolean = false;
          
          public var wingidonTargeter:Boolean = false;
          
          public var magikillTargeter:Boolean = false;
          
          public var flyingCrossbowmanTargeter:Boolean = false;
          ...
          public function elementalCombineMove() : Boolean
          {
               ...
               if(this.unit.team.type == Team.T_ELEMENTAL && this._currentCommand.type == UnitCommand.COMBINE)
               {
                    if(this.currentCommand.targetId in this.unit.team.game.units && this.unit.team.game.units[this.currentCommand.targetId] is Unit)
                    {
                         ...
                         if(this.unit.isAlive() && _loc1_ != null && _loc1_.isAlive() && _loc1_.isTargetable() && _loc1_.ai.currentCommand.type == UnitCommand.COMBINE && _loc1_.ai.currentCommand.targetId == this.unit.id)
                         {
          ...
          public function baseUpdate(param1:StickWar) : void
          {
               ...
               else if(this.mayMoveToAttack && this.unit.sqrDistanceTo(_loc2_) < 640000 && !this.unit.isGarrisoned)
               {
                    ...
                    {
                         if(this.unit.team.isAi || this.isSingleAttackCommand)
                         {
                              _loc3_ = 0;
          ...
          protected function checkForUnitAttack(param1:StickWar) : Unit
          {
               ...
               if(this.currentCommand is MoveCommand)
               {
                    _loc2_ = MoveCommand(this.currentCommand).realX;
                    _loc3_ = MoveCommand(this.currentCommand).realY;
                    ...
                    if(this.currentCommand.targetId in param1.units && param1.units[this.currentCommand.targetId] is Unit)
                    {
                         ...
                         if(param1.units[this.currentCommand.targetId].isFlying() && !this.unit.canAttackAir())
                         {
                              return null;
                         }
          ...
          public function getClosestUnitTarget() : Unit
          {
               ...
               var closestArcher:Archer = null;
               var closestSpearton:Spearton = null;
               var closestKnight:Knight = null;
               var closestDead:Dead = null;
               var closestMagikill:Magikill = null;
               var closestWingidon:Wingidon = null;
               var closestflyingCrossbowman:* = null;
               ...
               if(this.speartonTargeter)
               {
                    for each(_loc4_ in this.unit.team.enemyTeam.units)
                    {
                         if(_loc4_.type == Unit.U_SPEARTON && !_loc4_.isDead)
                         {
                              if(_loc4_.maxHealth == 900)
                              {
                                   if(!closestSpearton)
                                   {
                                        closestSpearton = _loc4_;
                                   }
                                   if(Math.abs(_loc4_.px - this.unit.px) < Math.abs(closestSpearton.px - this.unit.px))
                                   {
                                        closestSpearton = _loc4_;
                                   }
                              }
                    ...
                    return closestSpearton;
               }
               if(this.archerTargeter)
               {
                    for each(_loc4_ in this.unit.team.enemyTeam.units)
                    {
                         if(_loc4_.type == Unit.U_ARCHER && !_loc4_.isDead)
                         {
                              if(!closestArcher)
                              {
                                   closestArcher = _loc4_;
                              }
                              if(Math.abs(_loc4_.px - this.unit.px) < Math.abs(closestArcher.px - this.unit.px))
                              {
                                   closestArcher = _loc4_;
                              }
                         }
               ...
               if(this.deadTargeter)
               {
                    for each(_loc4_ in this.unit.team.enemyTeam.units)
                    {
                         if(_loc4_.type == Unit.U_DEAD && !_loc4_.isDead)
                         {
                              if(!closestDead)
                              {
                                   closestDead = _loc4_;
                              }
                              if(Math.abs(_loc4_.px - this.unit.px) < Math.abs(closestDead.px - this.unit.px))
                              {
                                   closestDead = _loc4_;
                              }
                         }
               ...
               if(this.knightTargeter)
               {
                    for each(_loc4_ in this.unit.team.enemyTeam.units)
                    {
                         if(_loc4_.type == Unit.U_KNIGHT && !_loc4_.isDead)
                         {
                              if(!closestKnight)
                              {
                                   closestKnight = _loc4_;
                              }
                              if(Math.abs(_loc4_.px - this.unit.px) < Math.abs(closestKnight.px - this.unit.px))
                              {
                                   closestKnight = _loc4_;
                              }
                         }
               ...
               if(this.magikillTargeter)
               {
                    for each(_loc4_ in this.unit.team.enemyTeam.units)
                    {
                         if(_loc4_.type == Unit.U_MAGIKILL && !_loc4_.isDead)
                         {
                              if(!closestMagikill)
                              {
                                   closestMagikill = _loc4_;
                              }
                              if(Math.abs(_loc4_.px - this.unit.px) < Math.abs(closestMagikill.px - this.unit.px))
                              {
                                   closestMagikill = _loc4_;
                              }
                         }
                    }
                    return closestMagikill;
               }
               if(this.wingidonTargeter)
               {
                    for each(_loc4_ in this.unit.team.enemyTeam.units)
                    {
                         if(_loc4_.type == Unit.U_WINGIDON && !_loc4_.isDead)
                         {
                              if(!closestWingidon)
                              {
                                   closestWingidon = _loc4_;
                              }
                              if(Math.abs(_loc4_.px - this.unit.px) < Math.abs(closestWingidon.px - this.unit.px))
                              {
                                   closestWingidon = _loc4_;
                              }
                         }
                    }
                    return closestWingidon;
               }
               if(this.flyingCrossbowmanTargeter)
               {
                    for each(_loc4_ in this.unit.team.enemyTeam.units)
                    {
                         if(_loc4_.type == Unit.U_FLYING_CROSSBOWMAN && !_loc4_.isDead)
                         {
                              if(!closestFlyingCrossbowman)
                              {
                                   closestFlyingCrossbowman = _loc4_;
                              }
                              if(Math.abs(_loc4_.px - this.unit.px) < Math.abs(0 - this.unit.px))
                              {
                                   closestFlyingCrossbowman = _loc4_;
                              }
                         }
                    }
                    return closestFlyingCrossbowman;
               }
               ...
               {
                    _loc1_ = this.unit.sqrDistanceToTarget(this.currentTarget);
               }
               ...
               while(_loc2_ < 3)
               {
                    ...
                    if(!((_loc4_ = this.unit.team.enemyTeam.units[_loc3_]).pz != 0 && !this.unit.canAttackAir()))
                    {
                         ...
                         {
                              if((_loc5_ = this.unit.sqrDistanceToTarget(_loc4_)) * 1.1 < _loc1_ && Unit(this.unit.team.enemyTeam.units[_loc3_]).isTargetable())
                              {
                                   _loc1_ = _loc5_;
          ...
          public function getClosestUnitTargetToFreeze() : Unit
          {
               ...
               while(_loc2_ < 3)
               {
                    ...
                    if(!((_loc4_ = this.unit.team.enemyTeam.units[_loc3_]).pz != 0 && !this.unit.canAttackAir()))
                    {
                         ...
                         {
                              if((_loc5_ = this.unit.sqrDistanceToTarget(_loc4_)) * 1.3 < _loc1_ && Unit(this.unit.team.enemyTeam.units[_loc3_]).isTargetable() && !Unit(this.unit.team.enemyTeam.units[_loc3_]).isFreezeLocked(this.unit))
                              {
                                   _loc1_ = _loc5_;
                                   this.currentTarget = this.unit.team.enemyTeam.units[_loc3_];
                              }
          ...
          public function getClosestTarget() : Unit
          {
               ...
               var closestMagikill:Magikill = null;
               var closestWingidon:Wingidon = null;
               var closestFlyingCrossbowman:FlyingCrossbowman = null;
               var closestSpearton:Spearton = null;
               var closestKnight:Knight = null;
               var closestDead:Dead = null;
               if(this.archerTargeter)
               ...
               if(this.flyingCrossbowmanTargeter)
               {
                    for each(_loc4_ in this.unit.team.enemyTeam.units)
                    {
                         if(_loc4_.type == Unit.U_FLYING_CROSSBOWMAN && !_loc4_.isDead)
                         {
                              ...
                              if(Math.abs(_loc4_.px - this.unit.px) < Math.abs(closestFlyingCrossbowman.px - this.unit.px))
                              {
                                   closestFlyingCrossbowman = _loc4_;
                              }
                         }
