package com.brockw.stickwar.engine.projectile
{
     ...
     public class Boulder extends Projectile
     {
          ...
          override protected function arrowHit(param1:Unit) : void
          {
               ...
               {
                    if(Unit(param1).checkForHitPointArrow(p1,py,intendedTarget) || Unit(param1).checkForHitPointArrow(p2,py,intendedTarget) || Unit(param1).checkForHitPointArrow(p3,py,intendedTarget))
                    {
                         unitNotToHit = param1;
                         hasHit = true;
               ...
               else if(unitNotToHit != null)
               {
                    ...
                    if(!Unit(unitNotToHit).checkForHitPoint(p3,Unit(unitNotToHit)) || _loc9_ > lastDistanceToCentre)
                    {
                         ...
                         if(unitNotToHit is Unit)
                         {
                              ...
                              Unit(unitNotToHit).setFire(this.burnFrames,this.burnDamage);
                              Unit(unitNotToHit).poison(this.poisonDamage);
                              Unit(unitNotToHit).slow(this.slowFrames);
                         }
                         if(this is Boulder)
                         {
