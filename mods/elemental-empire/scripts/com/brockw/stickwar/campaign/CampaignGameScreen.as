package com.brockw.stickwar.campaign
{
     ...
     public class CampaignGameScreen extends GameScreen
     {
          ...
          public var cheatMode:Boolean = false;
          ...
          override public function enter() : void
          {
               ...
               game.initTeams(Team.getIdFromRaceName(_loc1_.player.race),Team.getIdFromRaceName(_loc1_.oponent.race),_loc1_.player.statueHealth,_loc1_.oponent.statueHealth,null,null,1,_loc1_.insaneModifier,1,_loc6_,1,_loc7_);
               team = game.teamA;
               game.team = team;
               ...
               if(Team.getIdFromRaceName(this.main.campaign.getCurrentLevel().oponent.race) == Team.T_GOOD && main.campaign.currentLevel > 3)
               {
          ...
          {
               if(cheatMode && userInterface.keyBoardState.isDown(78) && userInterface.keyBoardState.isShift)
               {
                    game.teamB.statue.health = 0;
               }
               if(cheatMode && userInterface.keyBoardState.isDown(75) && userInterface.keyBoardState.isShift)
               {
                    game.teamB.statue.health = game.teamB.statue.maxHealth / 3;
               }
               if(cheatMode && userInterface.keyBoardState.isDown(77) && userInterface.keyBoardState.isShift)
               {
                    this.team.statue.health = this.team.statue.maxHealth / 2.6;
               }
               if(userInterface.keyBoardState.isDown(76) && userInterface.keyBoardState.isDown(77) && userInterface.keyBoardState.isDown(65) && userInterface.keyBoardState.isDown(79) && userInterface.keyBoardState.isShift)
               {
                    cheatMode = true;
               }
               if(cheatMode)
               {
                    game.teamA.gold += 500;
                    game.teamA.mana += 50;
                    game.gameScreen.userInterface.helpMessage.showMessage("Lmao is correct! Cheat Mode on!");
                    instaBuild(game.teamA);
               }
               if(this.doAiUpdates)
               {
          ...
          override public function endGame() : void
          {
               ...
               if(_loc1_.winner == team.id)
               {
                    main.campaign.campaignPoints = 0;
               ...
               {
                    main.postGameScreen.setTipText(main.campaign.getCurrentLevel().tipInsane);
               }
               else if(main.campaign.difficultyLevel == Campaign.D_HARD)
               {
                    main.postGameScreen.setTipText(main.campaign.getCurrentLevel().tipHard);
               }
               else if(main.campaign.difficultyLevel == Campaign.D_NORMAL)
               {
