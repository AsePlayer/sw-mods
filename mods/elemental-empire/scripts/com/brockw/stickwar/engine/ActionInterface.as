package com.brockw.stickwar.engine
{
     ...
     public class ActionInterface extends Sprite
     {
          ...
          public function drawCoolDown(param1:MovieClip, param2:Number) : void
          {
               ...
               var _loc6_:int = BOX_HEIGHT;
               var _loc7_:int = BOX_WIDTH;
               _loc3_.x = -_loc7_ / 2;
               _loc3_.y = -_loc6_ / 2;
          ...
          public function drawToggle(param1:MovieClip, param2:Boolean) : void
          {
               ...
               param1.addChild(_loc3_);
               _loc3_.graphics.clear();
               var _loc5_:int = BOX_HEIGHT;
               var _loc6_:int = BOX_WIDTH;
               _loc3_.x = -_loc6_ / 2;
               _loc3_.y = -_loc5_ / 2;
               if(param2)
               {
                    ...
                    for(; _loc2_ < this.currentActions.length; _loc2_++)
                    {
                         ...
                         else
                         {
                              if(UnitCommand(this.actions[this.currentActions[_loc2_]]).hasCoolDown)
                              {
                                   ...
                                   _loc7_ = UnitCommand(this.actions[this.currentActions[_loc2_]]).coolDownTime(param1.userInterface.selectedUnits.unitTypes[this.currentEntity.type][0]);
                                   _loc8_ = 1;
                                   while(_loc8_ < param1.userInterface.selectedUnits.unitTypes[this.currentEntity.type].length)
                                   {
                                        if((_loc9_ = UnitCommand(this.actions[this.currentActions[_loc2_]]).coolDownTime(param1.userInterface.selectedUnits.unitTypes[this.currentEntity.type][_loc8_])) < _loc7_)
                                        {
                                             _loc7_ = _loc9_;
                                        }
                                        _loc8_++;
               ...
               if(this._currentMove == null || this._currentMove != null && !this.clicked || this._currentMove == UnitCommand(this.actions[UnitCommand.MOVE]) || this.clicked)
               {
                    ...
                    while(_loc10_ < this.currentActions.length)
                    {
                         if(this.currentActions[_loc10_] < 0)
                         {
                              ...
                              if(param1.userInterface.keyBoardState.isDownForAction(_loc11_.hotKey) || param1.userInterface.mouseState.clicked && MovieClip(this.actionsToButtonMap[this.currentActions[_loc10_]]).hitTestPoint(param1.stage.mouseX,param1.stage.mouseY,false))
                              {
                         ...
                         else
                         {
                              ...
                              if(UnitCommand(this.actions[this.currentActions[_loc10_]]).isActivatable)
                              {
                                   ...
                                   if(param1.userInterface.keyBoardState.isDownForAction(UnitCommand(this.actions[this.currentActions[_loc10_]]).hotKey) || param1.userInterface.mouseState.clicked && MovieClip(this.actionsToButtonMap[this.currentActions[_loc10_]]).hitTestPoint(param1.stage.mouseX,param1.stage.mouseY,false))
                                   {
                                        param1.userInterface.mouseState.clicked = false;
          ...
          private function setUpActions() : void
          {
               ...
               this.actions[new SummonDeadCommand(this._game).type] = new SummonDeadCommand(this._game);
               ...
               this.actions[new FreezingCommand(this._game).type] = new FreezingCommand(this._game);
