package com.brockw.stickwar.engine.Ai.command
{
     ...
     public class UnitCommand
     {
          ...
          public static const FREEZING:int = 63;
          
          public static const SUMMON_DEAD:int = 64;
          ...
          public function prepareNetworkedMove(param1:GameScreen) : *
          {
               ...
               for(_loc5_ in param1.team.units)
               {
                    if(this._isSingleSpell)
                    {
                         ...
                         {
                              _loc12_ = (Number(Unit(param1.team.units[_loc5_]).px) - this.realX) * (Number(Unit(param1.team.units[_loc5_]).px) - this.realX) + (Number(Unit(param1.team.units[_loc5_]).py) - this.realY) * (Number(Unit(param1.team.units[_loc5_]).py) - this.realY);
                              if(_loc3_ == null)
                              {
               ...
               _loc6_ = Number(param1.userInterface.hud.hud.map.mouseX) / Number(param1.userInterface.hud.hud.map.width);
               _loc7_ = Number(param1.userInterface.hud.hud.map.mouseY) / Number(param1.userInterface.hud.hud.map.height);
               _loc9_ = (_loc8_ = param1.userInterface.hud.hud.map.globalToLocal(new Point(param1.userInterface.mouseState.mouseDownX,param1.userInterface.mouseState.mouseDownY))).x / Number(param1.userInterface.hud.hud.map.width);
               _loc10_ = _loc8_.y / Number(param1.userInterface.hud.hud.map.height);
