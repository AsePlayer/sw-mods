     ...
     public class RangedAi extends UnitAi
     {
          ...
          override public function update(param1:StickWar) : void
          {
               ...
               else if(mayMoveToAttack && currentTarget != null && unit.sqrDistanceTo(currentTarget) < 150000 && !unit.isGarrisoned)
               {
                    ...
                    if(this.mayKite && Math.abs(currentTarget.px - unit.px) < 450 && !RangedUnit(unit).isLoaded())
                    {
                         unit.walk(Util.sgn(unit.px - currentTarget.px) * Number(5),0,Util.sgn(unit.px - currentTarget.px));
                    }
