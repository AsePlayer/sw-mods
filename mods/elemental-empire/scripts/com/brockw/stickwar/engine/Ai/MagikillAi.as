package com.brockw.stickwar.engine.Ai
{
     ...
     public class MagikillAi extends UnitAi
     {
          ...
          override public function update(param1:StickWar) : void
          {
               ...
               if(currentCommand.type == UnitCommand.FREEZING || currentCommand.type == UnitCommand.NUKE || currentCommand.type == UnitCommand.STUN || currentCommand.type == UnitCommand.POISON_DART)
               {
                    if(!this.currentCommand.inRange(unit))
                    {
                         unit.mayWalkThrough = true;
                         unit.isBusyForSpell = true;
                         unit.walk((Number(currentCommand.realX) - Number(unit.px)) / 100,(Number(currentCommand.realY) - Number(unit.py)) / 100,(Number(currentCommand.realX) - Number(unit.px)) / 100);
                    }
                    ...
                    else if(currentCommand.type == UnitCommand.FREEZING)
                    {
                         Magikill(unit).freezingSpell(FreezingCommand(currentCommand).realX,FreezingCommand(currentCommand).realY);
                         nextMove(param1);
                    }
               }
               else
