package com.brockw.stickwar.engine.Ai
{
     ...
     public class UnitAi
     {
          ...
          public function getClosestUnitTarget() : Unit
          {
               ...
               var _loc4_:Unit = null;
               var _loc5_:Number = NaN;
               var closestMinerElement:ChromeElement = null;
               ...
               if(this.minerTargeter)
               {
                    ...
                    {
                         if(_loc4_.type == Unit.U_CHROME_ELEMENT && !_loc4_.isDead)
                         {
                              if(!closestMinerElement)
                              {
                                   closestMinerElement = _loc4_;
                              }
                              if(Math.abs(_loc4_.px - this.unit.px) < Math.abs(closestMinerElement.px - this.unit.px))
                              {
                                   closestMinerElement = _loc4_;
                              }
                         }
                    }
                    return closestMinerElement;
               }
               this.isTargeted = false;
               var _loc2_:int = 0;
               while(_loc2_ < 3)
