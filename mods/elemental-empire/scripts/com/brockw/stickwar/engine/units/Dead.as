package com.brockw.stickwar.engine.units
{
     ...
     public class Dead extends RangedUnit
     {
          ...
          override public function update(param1:StickWar) : void
          {
               ...
               super.update(param1);
               updateCommon(param1);
               if(this.aType == "a" && !this.setupComplete)
               {
                    maxHealth = 72;
                    health = 72;
                    this.arrowDamage = 2;
                    this.projectileVelocity = 25;
                    maxHealth = maxHealth;
               ...
               if(this.aType == "a")
               {
                    damage(com.brockw.stickwar.engine.units.Unit.D_NO_BLOOD | com.brockw.stickwar.engine.units.Unit.D_NO_SOUND,0.05,null);
               }
               if(this.aType != "")
               {
                    isCustomUnit = true;
               }
               if(this.bType == "b" && !this.setupComplete)
               {
                    ...
                    else if(_state == S_ATTACK)
                    {
                         if(MovieClip(_mc.mc).currentFrame == 31 && !hasHit)
                         {
                              ...
                              if(this.target != null)
                              {
                                   ...
                                   if(_loc6_ && team.mana > this.poisonMana || Boolean(this._isCastleArcher))
                                   {
                                        _loc5_ = Number(this.poisonDamageAmount);
                                   }
                                   if(mc.scaleX < 0)
                                   {
               ...
               if(this.isCastleArcher)
               {
                    Dead.setItem(mc,"Default","Jason Mask","Default");
               }
               else if(this.aType == "a")
               {
                    Dead.setItem(mc,"Default","Wrapped Helmet","Default");
               }
