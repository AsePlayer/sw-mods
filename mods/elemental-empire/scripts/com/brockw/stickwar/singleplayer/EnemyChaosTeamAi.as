package com.brockw.stickwar.singleplayer
{
     ...
     public class EnemyChaosTeamAi extends EnemyTeamAi
     {
          ...
          private var summonDeadSpell:SummonDeadCommand;
          ...
          public function EnemyChaosTeamAi(param1:Team, param2:BaseMain, param3:StickWar, param4:* = true)
          {
               ...
               this.summonDeadSpell = new SummonDeadCommand(param3);
          ...
          override protected function updateSpellCasters(param1:StickWar) : void
          {
               ...
               this.updateEclipsor(param1);
               this.updateMedusa(param1);
               this.updateChaosTech(param1);
               team.mana = _loc2_;
          }
          
          private function updateChaosTech(param1:StickWar) : void
          {
          ...
          private function updateEclipsor(param1:StickWar) : void
          {
               var _loc2_:Wingidon = null;
               var _loc3_:Unit = null;
               for each(_loc2_ in team.unitGroups[Unit.U_WINGIDON])
               {
          ...
          private function updateSkelator(param1:StickWar) : void
          {
               ...
               var _loc3_:Unit = null;
               var _loc4_:Statue = null;
               var _loc5_:Unit = team.enemyTeam.unitGroups[Unit.U_CHROME_ELEMENT][0];
               team.tech.isResearchedMap[Tech.SKELETON_FIST_ATTACK] = true;
               for each(_loc2_ in team.unitGroups[Unit.U_SKELATOR])
               {
                    _loc3_ = _loc2_.ai.getClosestTarget();
                    _loc4_ = this.team.enemyTeam.statue;
                    if(_loc2_.summonDeadCooldown() == 0)
                    {
                         this.summonDeadSpell.realX = _loc2_.px + 200;
                         this.summonDeadSpell.realY = _loc2_.py;
                         if(this.summonDeadSpell.inRange(_loc2_))
                         {
                              _loc2_.summonDead(_loc4_.px,_loc4_.py);
                         }
                    }
                    if(_loc3_)
                    {
                         ...
                         {
                              if(Math.abs(_loc5_.px - _loc2_.px) < 860)
                              {
                                   this.reaperSpell.targetId = _loc5_.id;
                                   this.reaperSpell.realX = _loc5_.px;
                                   this.reaperSpell.realY = _loc5_.py;
                                   if(this.reaperSpell.inRange(_loc2_))
                                   {
                                        _loc2_.reaperAttack(_loc5_);
                                   }
                              }
                              else
                              {
