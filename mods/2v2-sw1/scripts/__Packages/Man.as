class Man
{
     ...
     {
          var _loc4_ = this.game.nextUnitIndex();
          this.game.screen.attachMovie(movieName,movieName + _loc4_,this.game.getManBaseDepth() + _loc4_,{_x:this.x,_y:this.y});
          this.clip = this.game.screen[movieName + _loc4_];
          this.clip.name = movieName + _loc4_;
          this.clip._x = -100;
          this.clip._y = this.y;
          this.name = movieName + _loc4_;
          this.baseScale = 1700;
     ...
     function update()
     {
          ...
          var _loc7_ = this.game.getPartitionManager().getEnemyTeam(this,this.x);
          ...
          if(this.health <= 0 && this.dieTime == undefined)
          {
               ...
               if(this.isAlive)
               {
                    ...
                    var _loc10_ = int(Math.random() * 4) + 4;
                    _root.soundManager.playSound("pain" + _loc10_,this.x);
          ...
          {
               _loc8_ = _loc7_[_loc6_].getClip();
               if(this.isMenHitTest(this,_loc7_[_loc6_],0,0) && _loc7_[_loc6_].getSquad().getTeamName() != this.getSquad().getTeamName())
               {
          ...
          if(this.dx != 0)
          {
               ...
               {
                    _loc8_ = _loc7_[_loc6_].getClip();
                    if(this.isMenHitTest(this,_loc7_[_loc6_],this.dx,0))
                    {
                         _loc3_ = true;
                         _loc11_ = _loc7_[_loc6_];
                    }
               }
               if(!this.willHitCastles(this.dx) && this.x + this.dx > -100 && this.x + this.dx < this.game.getRIGHT() + 100 && !_loc3_ || _loc4_)
               {
                    this.x += this.dx;
               }
               else if(_loc3_ && !_loc4_)
               {
                    var _loc12_ = _loc11_.getDx() * _loc11_.getMASS() + this.dx * this.MASS;
                    if(_loc4_)
                    {
                         _loc11_.setDx(this.POWER / _loc11_.getMASS() * this.currentDirection);
                    }
                    else
                    {
                         _loc12_ /= this.MASS + _loc11_.getMASS();
                         _loc11_.setDx(_loc12_);
                         this.setDx(_loc12_);
                    }
               }
          }
          if(_loc4_)
          {
               if(this.x < _loc5_.getX())
               {
                    _loc5_.setDx(_loc5_.getDx() + 0.5);
               }
               else
               {
                    _loc5_.setDx(_loc5_.getDx() - 0.5);
               }
               if(this.y < _loc5_.getY())
               {
                    _loc5_.setDy(_loc5_.getDy() + 0.5);
               }
               else
               {
                    _loc5_.setDy(_loc5_.getDy() - 0.5);
          ...
          if(this.dy != 0)
          {
               for(_loc6_ in _loc7_)
               {
                    if(this.isMenHitTest(this,_loc7_[_loc6_],0,this.dy))
                    {
                         _loc3_ = true;
                         _loc7_[_loc6_].setDy(this.POWER / _loc7_[_loc6_].getMASS() * this.currentDirection);
                    }
               }
               if(this.y + this.dy < this.game.getBOTTOM() && this.y + this.dy > this.game.getTOP() && !_loc3_ || _loc4_)
               {
                    this.y += this.dy;
               }
          }
     ...
     function damage(amount, direction, type)
     {
          ...
          this.action();
          var _loc5_ = amount * amount / this.MASS * direction * this.DAMAGE_MOMENTOM_CONSTANT;
          ...
          _loc6_ = "healthLost" + this.healthLostnum;
          this.game.screen.attachMovie("healthLost",_loc6_,10000000000040 + this.healthLostnum);
          this.game.screen[_loc7_]._x = this.x;
          this.game.screen[_loc7_]._y = this.y - 100;
          this.game.screen[_loc7_].gold.gold.text = "- " + int(_loc6_);
     ...
     function isMenHitTest(a, b, dx, dy)
     {
          ...
          if(Math.abs(b.getY() - a.getY() - dy) > 8 * _loc5_)
          {
               return false;
          }
